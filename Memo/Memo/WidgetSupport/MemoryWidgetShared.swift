...
enum MemoryWidgetShared {
    ...
    static func payloadURL() throws -> URL {
        try sharedContainerURL().appendingPathComponent(payloadFileName)
    }

    static func imageURL() throws -> URL {
        try sharedContainerURL().appendingPathComponent(imageFileName)
    }
}
